...
class UsersController extends AppController
{
        ...
        if ($this->request->is("post", "put"))
        {
            ...
            if ($result)
            {
                ...
                try
                {
                    ...
                    $this->request->data["Company"]['is_active'] = 1;
    ...
    private function _signup_send_email($id)
    {
        ...
        $template = $this->EmailTemplate->find("first", array(
            "conditions" => array(
                "code" => EmailTypes::ACCOUNT_ACTIVATE
            )
        ));
        ...
        $this->email(EmailTypes::ACCOUNT_ACTIVATE, $to_email, FROM_EMAIL, $subject, $content);
    }
    ...
    public function forgot_password()
    {
        ...
        if ($this->request->is('post'))
        {
            ...
            else
            {
                ...
                $template = $this->EmailTemplate->find("first", array(
                    "conditions" => array(
                        "code" => EmailTypes::FORGOT_PASSWORD
                    )
                ));

                $placeholder_values['User.firstname'] = $user['firstname'];
                $placeholder_values['User.lastname'] = $user['lastname'];
                $placeholder_values['User.password'] = $new_password;
    ...
    public function initDB()
    {
        ...
        $this->Acl->allow($group, 'controllers/Companies/admin_block');
        $this->Acl->allow($group, 'controllers/Companies/admin_unblock');
        $this->Acl->allow($group, 'controllers/EmailPlaceholders');
        $this->Acl->allow($group, 'controllers/EmailTemplates');
